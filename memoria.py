...
class Historico:
    ...
    def get_name_livro_all(self):
        livros =open(self.path+"li.txt").read().strip().split("\n")
        return livros
    def get_cargo_apresentador(self):
        cargos = open(self.path + "cargos.txt").read().strip().split("\n")
        return cargos
    def get_name_apresentador(self):
        people_names = open(self.path + "people_names.txt").read().strip().split("\n")
        return people_names
    def set_cargo_apresentador(self,cargo):
        with open(self.path + "cargos.txt","a") as info:
            info.write(cargo+"\n")
    def set_name_apresentador(self,name):
        ...
        if name in names:
            ...
            print("nome ja existe")
        with open(self.path + "people_names.txt","a") as info:
            info.write(name+"\n")
    def delete_people_name(self,name):
        cargos = open(self.path + "people_names.txt").read().strip().split("\n")
        if name in cargos:
            del cargos[cargos.index(name)]
        with open(self.path + "people_names.txt","w") as info:
            for c in cargos:
                info.write(c+"\n")
